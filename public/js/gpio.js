...
(function () {
    ...
    function loadRules() {
        // Rules are loaded with pin status
    }
                ...
                <div class="col-6 col-sm-4 col-md-3 col-lg-2">
                    <div class="pin-card ${isSelected ? 'selected' : ''}" data-pin="${pin.pin}" 
                        ...
                        <div class="pin-footer">
                            <div class="small text-muted text-truncate">${config.mode}</div>
    ...
    function openPinModal(pin) {
        ...
        document.getElementById('modalPinName').value = data.config.name || `GPIO${pin}`;
    ...
    function writePin(pin, value, type = 'digital', frequency = 1000) {
    ...
    function readPin(pin) {
        const type = document.getElementById('modalPinMode').value.includes('analog') ? 'analog' : 'digital';
    ...
    function getSpecialPinNote(pin) {
        const special = ESP32_S3_PINS.special.find(p => p.pin === pin);
        return special?.note || null;
    }
    ...
    function attachEventListeners() {
        // Pin modal
        document.getElementById('savePinBtn')?.addEventListener('click', applyPinSettings);
